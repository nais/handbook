...
gcloud organizations add-iam-policy-binding "$ORG_ID" --member="domain:nais.io" --role="roles/compute.osLoginExternalUser" && echo "✔️ nais.io domain allowed to log into VMs" || echo "❌ Failed to allow nais.io domain to log into VMs" && exit 1

echo "Creating the Nais folder"
gcloud resource-manager folders create --display-name=nais --organization="$ORG_ID" && echo "✔️ Nais folder created" || echo "❌ Failed to create the Nais folder" && exit 1
